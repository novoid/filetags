...
The command =register_filetags_for_send_to_folder= does the following
steps for you. You can have a look at its options by starting a
=cmd.exe= shell and typing in:

: register_filetags_for_send_to_folder -h
